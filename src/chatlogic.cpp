...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; }); //deleted for task 3
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            */
                            ...
                            // create new edge
                            //GraphEdge *edge = new GraphEdge(id);  deleted for task 4
                            ...
                            */
                            ...
                            //_edges.push_back(std::move(edge));   //deleted for task 4
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                //rootNode = *it; // assign current node to root    //deleted for task 3
                rootNode = it->get();
            }
            else
            {
    ...
    //_chatBot->SetRootNode(rootNode);    //deleted for task 5
    chatBot.SetRootNode(rootNode);
    //rootNode->MoveChatbotHere(_chatBot);     //deleted for task 5
